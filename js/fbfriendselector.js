...
var FBFriendSelector = (function(module, $) {
  ...
  newInstance = function(options) {
    ...
    var showFriendSelector, hideFriendSelector, getselectedFriendIds, setDisabledFriendIds, filterFriends, reset,
      ...
      // Private variables
      instanceSettings, selectedFriendIds = [],
      disabledFriendIds = [],
    ...
    bindEvents = function() {
      ...
      $buttonOK.bind('click', function(e) {
        ...
        if (typeof instanceSettings.callbackSubmit === "function") {
          instanceSettings.callbackSubmit(selectedFriendIds);
        }
      });
  ...
  buildFriendSelector = function(batchImageSize, callback) {
    ...
    buildFriendMarkup = function(friend, loadImage) {
      ...
      html = '<li__LI_ATTR__>' + '<a href="#" class="clearfix" id="friend__FRIEND_ID__">' + '<div>' + '<img src="__IMG_SRC__" __IMG_DATA__ width="30" height="30" alt="__FRIEND_NAME__" class="FBFriendSelector_friendAvatar" />' + '<input type="checkbox" name="friends[__FRIEND_ID__]" value="__FRIEND_ID__" />' + '<div>__FRIEND_NAME__</div>' + '</div>' + '</a>' + '</li>';
